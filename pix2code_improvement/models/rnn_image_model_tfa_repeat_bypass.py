...
from tensorflow.python.keras.layers import Dense, Dropout,  concatenate, Flatten, LSTMCell, GRUCell, Embedding, MaxPooling2D, Conv2D

from tensorflow.keras.optimizers import RMSprop
from utils import preprocess_image
import numpy as np
from constants import START_WORD, PLACEHOLDER, IMAGE_SIZE
from models.shallow_cnn_model import ShallowCnnUnit
...
class CounterUnit(tf.keras.layers.Layer):
    def __init__(self, layer_size, activation='relu', dropout_ratio=0.25, **kwargs):
        super().__init__(**kwargs)
        self.main_layers = [
            Flatten(name='flatten'),
            Dense(layer_size, activation=activation, name="counter1"),
            Dropout(dropout_ratio, name='counter_drop_1'),
...
class RnnImageModelTfaRepeatBypass(tf.keras.models.Model):
    ...
    def __init__(self, words, image_count_words, *args, max_code_length, activation='relu', cell_type='lstm',
                 ...
                 order_layer_output_size=1024, embedding_size=None, pool_using_strides=False, **kwargs):
        ...
        self.pool_using_strides = pool_using_strides
        ...
        self.ordering_layers = [
            ...
            Dense(order_layer_output_size, activation=activation, name='ordering_final')
        ]
        self.flatten_img = Flatten(name='bypass_flatten')
        if pool_using_strides:
            self.pooling_conv_layers = [
                Conv2D(1, kernel_shape, padding='same', name=word + "_pool", activation=activation, strides=4) for word
                in image_count_words]
        else:
            self.max_pooling = MaxPooling2D(pool_size=(4, 4))
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        if self.pool_using_strides:
            max_pooled = [l(cnn_out) for l, cnn_out in zip(self.pooling_conv_layers, cnn_output_layers)]
        else:
            max_pooled = [self.max_pooling(l) for l in cnn_output_layers]
        max_pooled_flattened = self.flatten_img(concatenate(max_pooled))

        obj_counter_outputs = [self.parallel_counter_unit(img_out) for img_out in max_pooled]
        ...
        sampler = TrainingInferenceSampler(training=training, voc_size=self.voc_size,
                                           ...
                                           img_input=max_pooled_flattened)

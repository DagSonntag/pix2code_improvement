...
class RnnImageModelMemory(tf.keras.models.Model):
    ...
    def __init__(self, words, image_count_words, *args, max_code_length, activation='relu',
                 ...
                 order_layer_output_size=1024, **kwargs):
        ...
        self.memory_layer = tf.keras.layers.LSTM(order_layer_output_size, return_sequences=True)
        self.decoder_layers = [
    ...
    def call(self, inputs):
        ...
        init_state_h = self.memory_layer.get_initial_state(context_inp)[0]
        decoder_in = self.memory_layer(context_inp, initial_state=[init_state_h, img_out])

        for layer in self.decoder_layers:
            decoder_in = layer(decoder_in)

        outputs = obj_counter_ouputs
        outputs.update({'code': decoder_in})
